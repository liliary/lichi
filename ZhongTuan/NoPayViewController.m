...
// _topLayout = (UIView*)self.topLayoutGuide;
  ...
  // _belowtitleview = [[ZTTitleLabel alloc] initWithTitle:@"特卖未支付订单"];
...
-(void)initTableView{
   ...
   // _nopayListView = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    _nopayListView.separatorStyle = UITableViewCellSeparatorStyleNone;
  
   // _nopayListView.backgroundColor=[UIColor redColor];
...
NoPayCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
    if (!cell) {
        cell = [[NoPayCell  alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"cell"];//会放入到队列中的
    }
